...
# --- 1. AYARLAR ---
st.set_page_config(page_title="Mutabakat Pro V61", layout="wide")
    ...
    if f1:
        ...
        if not is_ins:
            ...
            if fcol1 and fcol1!="Seçiniz...":
                ...
                dv1 = get_default_multiselect(uv1, ["Satıcı ödemesi", "HAVALE", "KREDI KARTI", "EFT"])
    ...
    if f2:
        ...
        if is_ins:
            ...
            if fcol and fcol != "Seçiniz...":
                ...
                dv = get_default_multiselect(uv, ["HAVALE", "KREDI KARTI", "EFT"])
            ...
            if fcol2 and fcol2 != "Seçiniz...":
                uv2 = d2[fcol2].astype(str).unique().tolist()
                dv2 = get_default_multiselect(uv2, ["Ödeme Kanalı gelen ödeme", "HAVALE", "EFT"])
                fv2 = st.multiselect("Ödeme Olanlar (Ayır):", uv2, default=dv2, key="fvals_ch_2")
                cf2['odeme_turu_sutunu'] = fcol2
                cf2['odeme_turu_degerleri'] = fv2
    ...
    if f1 and f2:
        ...
        try:
            ...
            with st.spinner('İşleniyor...'):
                ...
                # --- OTOMATİK ÖDEME AYRIŞTIRMA (BİZ TARAFI İÇİN PATCH) ---
                # Eğer "Biz" tarafında manuel filtre seçilmediyse ama "Ödeme Ref" sütunu seçildiyse,
                # Referansı dolu olanları otomatik "Ödeme" havuzuna at.
                if not is_ins:
                    # Biz tarafı kontrolü
                    if pay_biz.empty and 'Payment_ID' in raw_biz.columns:
                        # Payment_ID'si olanları bul (Boş olmayanlar)
                        mask_is_payment = raw_biz['Payment_ID'].str.len() > 2
                        if mask_is_payment.any():
                            new_payments = raw_biz[mask_is_payment].copy()
                            # Ana listeden çıkar
                            raw_biz = raw_biz[~mask_is_payment].copy()
                            # Ödeme listesine ekle
                            pay_biz = pd.concat([pay_biz, new_payments], ignore_index=True)
                ...
                if not pay_biz.empty and not pay_onlar.empty:
                    ...
                    for idx, row in pay_onlar.iterrows():
                        ...
                        if pid and len(pid)>2:
                            if pid not in dict_pay_ref: dict_pay_ref[pid] = []
                            dict_pay_ref[pid].append(idx)
